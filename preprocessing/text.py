...
class Cleanner:
    ...
    def clean(self, sequences: list):
        for i in range(len(sequences)):
            sequences[i] = self.__clean(sequence=sequences[i])
...
class PhonemeHandler:
    ...
    def __convert(self, sequence: str):
        phoneme_list = " ".join(self.g2p(sequence))
        phoneme_list = re.sub(r"\s\s+", " ", phoneme_list)

        return phoneme_list

    def convert(self, sequences: list):
        for i in range(len(sequences)):
            sequences[i] = self.__convert(sequences[i])
...
class TextProcessor:
    def __init__(self, tokenizer_path: str = None) -> None:
        ...
        self.phoneme_handler = PhonemeHandler()
    ...
    def process(self, data: list, max_len: int = None, padding: str = 'post', truncating: str = "post", start_token: bool = False, end_token: bool = False) -> np.ndarray:
        ...
        sequences = self.phoneme_handler.convert(sequences)
        sequences = self.sign_handler.sign(sequences, start_token=start_token, end_token=end_token)
